   ...
   <div class="modal" id="addItemModal" tabindex="-1" role="dialog" style="display: none;">
                        ...
                        <tbody>
                            @foreach($data as $item)
								...
								<td style="border: 1px solid #ddd; padding: 8px;">{{ $item->id }}</td>
									...
									<td><a href="{{ route('event.delete', $item->id) }}" class="btn btn-danger delete-btn" data-item-id="{{ $item->id }}">Delete</a></td>

                                    <td><a href="{{ route('event.complete', $item->id) }}" class="btn btn-primary" data-item-id="{{ $item->id }}">complete</a></td>
	...
	<script>
        ...
        $(document).on('click', '.edit-btn', function () {
            ...
            $.ajax({
                type: 'GET',
                url: '{{ route("event.edit", ":id") }}'.replace(':id', itemId),
                dataType: 'json',
                // headers: {
                //             Authorization: 'Bearer ' + token
                //         },
                success: function (response) {
                    ...
                    $('#editmodel').find('[name="event_date_time"]').val(response.itemm.event_date_time);
                    $('#editmodel').find('[name="location"]').val(response.itemm.location);
                    $('#editmodel').find('[name="latitude"]').val(response.itemm.latitude);
                    $('#editmodel').find('[name="longitude"]').val(response.itemm.longitude);
                    $('#editmodel').find('[name="status"]').val(response.itemm.status);
        ...
        $('#editmodel').on('submit', 'form', function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route("event.update", ":id") }}'.replace(':id', itemId),            
                data: formData,
                // headers: {
                //             Authorization: 'Bearer ' + token
                //         },
